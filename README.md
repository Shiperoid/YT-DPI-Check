...
* Подробнее — в начале [CHANGELOG.md](CHANGELOG.md) (раздел **v2.3.3**, Windows).
...
* **Параллельный первый проход TLS (опц., по умолчанию ВЫКЛ).** В **2.3.2** переключатель был пунктом **6** в меню `[S]`; в **2.3.3** этот пункт занят настройкой **`targets.txt`** — сам флаг **`ScanParallelTlsFirstPass`** в конфиге по-прежнему поддерживается.
...
* Подробный разбор — [CHANGELOG.md](CHANGELOG.md#yt-dpi-v232-windows-yt-dpips1--yt-dpibat).

## Что нового в v2.3.2 (Bash, кратко)

Сборка **`YT-DPI.sh` с `SCRIPT_VERSION="2.3.2"`** соответствует линии релизов **2.3.2** на GitHub.
...
* Подробности — [CHANGELOG.md](CHANGELOG.md#yt-dpi-v232-bash-yt-dpish).
...
* **Релиз:** [Releases](https://github.com/Shiperoid/YT-DPI/releases) — для Windows нужны **`YT-DPI.bat`** и **`YT-DPI.ps1`** в одной папке; актуальный патч Windows — [v2.3.3](https://github.com/Shiperoid/YT-DPI/releases/tag/2.3.3) (или последний тег линии **`2.3.x`**).

Полный разбор: **v2.3.3** (Windows) и история **v2.3.2** — [CHANGELOG.md](CHANGELOG.md). Историческая база релиза 2.3.0 сохранена ниже в файле.
...
1. Скачайте **`YT-DPI.bat`** и **`YT-DPI.ps1`** со [страницы релиза v2.3.3](https://github.com/Shiperoid/YT-DPI/releases/tag/2.3.3) (или последнего релиза) и положите **в одну папку**.
